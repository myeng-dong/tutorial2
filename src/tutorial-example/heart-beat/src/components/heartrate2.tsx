...
export const Heartrate = () => {
    ...
    const transform = useDerivedValue(() => [{ scale: mix(beatEasing(1 - progress.value), 1.1, 1) }]);
    return (
        <Canvas style={{ flex: 1 }}>
            <Fill color="black" />
            {values.map((val, i) => (
            ...
            <Fill transform={transform} origin={origin}>
                <Shader source={source} uniforms={{ resolution: [width, height], c1, c2 }} />
            </Fill>
        </Canvas>
    );
};

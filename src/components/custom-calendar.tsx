...
export const CustomCalMine = () => {
    ...
    const renderCalendar = useCallback(() => {
        // 지난달 마지막날과 이번달 마지막날
        const prevLast = new Date(getYear, getMonth, 0); // 2024 03 31 토
        const thisLast = new Date(getYear, getMonth + 1, 0); // 2024 04 30
            ...
            return (
                <View style={{ flexDirection: 'row' }}>
                    {dates.slice(y * 7, (y + 1) * 7).map((date, i) => {
                        // 오늘인지
                        let isToday = year_today == getYear && month_today == getMonth && date == date_today;
                        // 비활성 날짜(첫일 말일 전달 후달 일자)
                        const noSelDate = (y == 0 && i < firstDateIndex) || (y > 2 && date < 7);
                        // 해당 달 활성날짜 중 이미 지난날짜
                        const disableNowMonthDate =
                            year_today == getYear && month_today == getMonth && date < date_today;
                        const disableLaterMonth = year_today >= getYear && month_today > getMonth;
                        // 고른날짜
                        let isSelect = selDay == date && !noSelDate && selMonth == getMonth + 1 && selYear == getYear;
                        // 비활성 날짜 및 주말 적용
                        const condition =
                            ...
                            <Pressable
                                ...
                                <View
                                    style={{
                                        ...
                                        overflow: 'hidden',
                                        backgroundColor: isSelect ? '#006934' : isToday ? '#E6F0EB' : 'transparent',
                                    }}>
                                    <Text
                                        style={{
                                            fontWeight: 600,

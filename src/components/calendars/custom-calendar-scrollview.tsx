...
export const CustomCalendarScrollView = () => {
    ...
    const [dateArr, setDateArr] = useState<number[]>([month_today - 1, month_today, month_today + 1]);
        ...
        (month: number) => {
            ...
            return (
                <View style={{ width: widthScale(375) }}>
                    {[0, 1, 2, 3, 4, 5, 6].map((week, y) => {
                        return (
                            <View style={{ flexDirection: 'row' }}>
                                {dates.slice(y * 7, (y + 1) * 7).map((date, i) => {
                                    // 오늘인지
                                    let isToday = year_today == getYear && month == month_today && date == date_today;
                                    // 비활성 날짜(첫일 말일 전달 후달 일자)
                                    const noSelDate = (y == 0 && i < firstDateIndex) || (y > 2 && date < 7);
                                    // 해당 달 활성날짜 중 이미 지난날짜
                                    const disableNowMonthDate =
                                        year_today == getYear &&
                                        month_today + month == month_today + month &&
                                        date < date_today;
                                    const disableLaterMonth =
                                        year_today >= getYear && month_today + month > month_today + month;
                                    // 고른날짜
                                    let isSelect =
                                        selDay == date &&
                                        !noSelDate &&
                                        selMonth == month_today + month + 1 &&
                                        selYear == getYear;
                                    // 비활성 날짜 및 주말 적용
                                    const condition = i == 0 ? 'red' : i == 6 ? 'blue' : '#222';
                                    return (
                                        <Pressable
                                            onPress={() => {
                                                if (noSelDate && date < 7) {
                                                ...
                                                if (disableLaterMonth) return;
                                                setSelDay(date);
                                                setSelMonth(month_today + month + 1);
                                                setSelYear(getYear);
                                            }}
                                            style={[
                                                {
    ...
    function handleScroll(event: NativeSyntheticEvent<NativeScrollEvent>): void {
        // distanceFromStart.x === 0이면 목록의 시작 부분에 도달합니다
    ...
    return (
        <View style={{ flex: 1, width: widthScale(375) }}>
            ...
            <FlatList
                ...
                renderItem={(item) => renderCalendar(item.index)}
                horizontal
                pagingEnabled
